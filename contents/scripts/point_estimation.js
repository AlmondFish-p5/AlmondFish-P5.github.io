...
function readHtmlSettings() {
  ...
  userDifference = document.querySelector('#user_difference').value * 1;
...
function plotOneSimulation(index) {
  ...
  // 一致していたら / 許容範囲内なら、正解として数える
  if (userDifference==0) {
    if (st.mean==userParameter.mu) hitNumber++;
  } else {
    if (Math.abs(st.mean - userParameter.mu) < userDifference) hitNumber++;
  }
  // 誤差の最小値を記録する
...
//        u2:calcMean(dat.u2),
//      };
      const st_str = `点推定「成功」回数：${hitNumber}回（${(hitNumber/userMaxTrial*100).toFixed(1)}％）\n誤差が最小の標本平均値：${minDev.toFixed(5)}`;
      textAlign(LEFT, TOP);
      textSize(16);
      fill(0);
      noStroke();
...
function makeDownloadData(id) {
  ...
  switch (id) {
    ...
    case 'script':
      ...
      data.push('# 標本平均と母平均の差を計算する');
      data.push('mu <- ' + userParameter.mu);
      data.push('s_size <- ' + userSampleSize);
      data.push('min_dif <- ' + userDifference);
      data.push('for (i in 1:NROW(df)) {');
      data.push('  df$dif[i] <- mu - mean(t(df[i ,1:s_size]))');
      data.push('}');
      data.push('plot(1:NROW(df), df$dif)');
      data.push('# 誤差範囲にあるものを数える');
      data.push('sum(abs(df$dif) < min_dif)');
      break;
    default:
      return null;
  }

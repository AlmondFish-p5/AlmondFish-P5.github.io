...
let userFramerate = 16 // 設定可能にしてもいい
...
function changeExecMode() {
  ...
  document.querySelector('#rvprob').disabled = !userManMode;
  flag = 0;
  rvdata = [];
  if (!userManMode) {
    trials = 0;
...
function readHtmlSettings() {
  ...
  userRvProb = document.querySelector('#rvprob').value * 1;
  param.prob = userRvProb;
...
// 確率が変更された
document.querySelector('#rvprob').addEventListener('change', ()=>{
  userRvProb = document.querySelector('#rvprob').value * 1;
  readHtmlSettings();
});
...
// 自動モードの結果表示
let db = new Array(userVarNumber);

function dispResult() {
  // 確率変数ごとの値の変化を記録する部分
  for (let i=0; i<userVarNumber; i++) {
    db[i] =[];
    for (let j=i; j<rvdata.length; j+=userVarNumber) {
      db[i].push(rvdata[j]);
    }
  }
  // 平均値を計算する部分
/*  let mean = new Array(userVarNumber).fill(0);
  ...
  */
  ...
  // 確率変数の組み合わせごとに、1-0の組み合わせ数を数える部分
  let freq = [];
  let mstr = '';
  const mpair = [['ウラ-ウラ： ', 'ウラ-表： '], ['表-ウラ： ', '表-表： ']];
  for (let i=0; i<userVarNumber-1; i++) {
    for (j=i+1; j<userVarNumber; j++) {
      freq = [[0,0],[0,0]];
      ...
      //console.log('i=',i,'j=',j);
      //console.log(freq);
      mstr += `コイン ${i+1}＆${j+1}　` + mpair[1][1] + freq[1][1] + '　　' + mpair[1][0] + freq[1][0] + '　　' + mpair[0][1] + freq[0][1] + '　　' + mpair[0][0] + freq[0][0] + '<br />'; 
    }
  }
  mstr = '<strong>【理 論 値】 　' + 
          mpair[1][1] + `${round(param.prob * param.prob * userTrialMax,1)}` + '　　' +
          mpair[1][0] + `${round(param.prob * (1-param.prob) * userTrialMax,1)}` + '　　' + 
          mpair[0][1] + `${round((1-param.prob) * param.prob * userTrialMax,1)}` + '　　' + 
          mpair[0][0] + `${round((1-param.prob) * (1-param.prob) * userTrialMax,1)}`+
          '</strong> <br />' + mstr; 
  ...
  document.querySelector('#result').innerHTML = '<p>' + pstr + '<hr>' + mstr +'</p>';
}

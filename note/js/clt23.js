...
class myCoinLinePlot {
  ...
  #iter;
  #label;
  constructor(x, y, w, h, size, ymax, iter, rvname, label='') {
    this.#margin = {
...
//      text(`${i} `, this.#gbase.x - 2, this.#gbase.f + this.#step.y * i);
      text(this.#label[i], this.#gbase.x - 2, this.#gbase.f + this.#step.y * i);
    }
    pop();
  }
  // --- データを受け取って描画
...
//      this.#cbase.y + this.#size / 2);
      text(this.#label[x], this.#cbase.x + (this.#size + this.#margin.ctoc) * n + this.#size / 2, 
      this.#cbase.y + this.#size / 2);
      pop();
    }
    this.#data.push(x);
  ...
  dispSummary(r=1) {
    let s = 0;
    this.#data.forEach(x => {s += x*r});
    const m = s / this.#data.length;
    let ss = 0;
    this.#data.forEach(x => {ss += ((x*r-m)**2)});
    const v = ss / this.#data.length;
    const sstr = `平均：${m.toFixed(3)}` + '  \n' + `分散：${v.toFixed(3)}` + '  ';
//console.log(m, v);
    push();
    textSize(15);
    textAlign(LEFT, TOP);
    fill(0);
    text(sstr, this.#gbase.x + this.#gbase.w + 20, this.#cbase.y + 5);
    pop();
  }
}
...
function initCanvas() {
  ...
  canvasHeight = clsHeight.margin + clsHeight.coin * userRvNumber + 
                 clsHeight.sum * rvOption.sum + 
                 clsHeight.mean * rvOption.mean + 20;
  if (userRvNumber>2) canvasHeight += (clsHeight.add * (userRvNumber-2));
  // キャンバス作成
  ...
  if (rvOption.sum) {
    y += 10;
    const heightAdd = userRvNumber>2 ? clsHeight.add * (userRvNumber-2) : 0;
    clsLinePlotSum = new myCoinLinePlot(xleft, y, clsWidth, clsHeight.sum+heightAdd, 
      rvSize, userRvNumber, 10, 'X_iの和');
    clsLinePlotSum.iter = userRvIter;
    y += clsHeight.sum + heightAdd;
  }
  if (rvOption.mean) {
    const axstr = [
      [''], ['0', '1'], ['0', '1/2', '1'], ['0', '1/3', '2/3', '1'],
      ['0', '1/4', '2/4', '3/4', '1'], ['0','1/5','2/5','3/5','4/5','1']
    ];
    y += 10;
    clsLinePlotMean = new myCoinLinePlot(xleft, y, clsWidth, clsHeight.mean, 
      rvSize, userRvNumber, 10, 'X_iの平均', axstr[userRvNumber]);
  ...
  if (loopOn) {
    ...
    if (trials>=userRvIter) {
      ...
      for (let i=0; i<userRvNumber; i++) {
        ...
        clsCoinLine[i].dispSummary();
      }
      clsLinePlotSum.dispStat();
      clsLinePlotSum.dispSummary();
      clsLinePlotMean.dispStat();
      const rt = 1 / userRvNumber;
      clsLinePlotMean.dispSummary(rt);
      loopOn = false;
      noLoop();
      //resetButtons();
    }

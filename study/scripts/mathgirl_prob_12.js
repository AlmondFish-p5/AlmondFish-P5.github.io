/** =====================================================
 *  数学ガールの秘密ノート／確率の冒険　研究問題1-2
 *  mathgirl_study_12.js
 */
...
let   cvGraph  = {x:  40, y: 250, w: 750, h: 200, f:450, sy:-200, sx:750/500,};
let   cvPlot   = {x:  40, y: 450, w: 750, h: 150, z:600, sy:150/50, sx:750/500,};
...
function initCanvas() {
  ...
  cvPlot.sx = cvGraph.sx;
  cvPlot.sy = -1*cvPlot.h/(userTrialMax*0.13);
  // キャンバス作成
...
const drawAxis = (n) =>{
  ...
  if (n<1) {
    ...
    line(cvPlot.x,  cvPlot.y+10, cvPlot.x,  cvPlot.z );
    // X軸
    line(cvGraph.x, cvGraph.f, cvGraph.x+cvGraph.w, cvGraph.f);
    line(cvPlot.x,  cvPlot.z,  cvPlot.x+cvPlot.w,   cvPlot.z );
    // prob表示線
    stroke('red');
    line(cvGraph.x, cvGraph.f+cvGraph.sy*userProbability, cvGraph.x+cvGraph.w, cvGraph.f+cvGraph.sy*userProbability);
    stroke('yellowgreen');
    line(cvPlot.x, cvPlot.z+userTrialMax*0.1*cvPlot.sy, cvPlot.x+cvPlot.w, cvPlot.z+userTrialMax*0.1*cvPlot.sy);
    line(cvPlot.x, cvPlot.z+userTrialMax*0.05*cvPlot.sy, cvPlot.x+cvPlot.w, cvPlot.z+userTrialMax*0.05*cvPlot.sy);
    ...
    text('0', cvPlot.x-2,  cvPlot.z-5);
    text(userTrialMax*0.1, cvPlot.x-2, cvPlot.z+userTrialMax*0.1*cvPlot.sy);
    text(userTrialMax*0.05, cvPlot.x-2, cvPlot.z+userTrialMax*0.05*cvPlot.sy);
  } else {
    strokeWeight(0.3);
    stroke(128);
    line(cvGraph.x+cvGraph.sx*n, cvGraph.y,   cvGraph.x+cvGraph.sx*n, cvGraph.f);
    line(cvPlot.x+cvPlot.sx*n,   cvPlot.y+10, cvPlot.x+cvPlot.sx*n,   cvPlot.z);
    noStroke();
    fill(128);
    textAlign(RIGHT,BASELINE);
    textSize(10);
...
const drawPlot = (n, p, q) => {
  push();
  stroke(p>=0 ? 'red' : 'blue');
  if (abs(p) > userTrialMax*0.13) stroke('black');
  p = min(abs(p), userTrialMax*0.13);
  strokeWeight(cvPlot.sx+0.1);
  line(cvPlot.x+cvPlot.sx*n, cvPlot.z, cvPlot.x+cvPlot.sx*n, cvPlot.z+cvPlot.sy*p);
  pop();
};
...
function draw() {
  ...
  // 1回ごとの統計データを表示する
    ...
    drawPlot(coinCounter, coinDiff[coinCounter-1]);
    // 指定した回数投げ終わったか
